...
const growth = [
 {name:'Wild growth',heights:[1,2.8,6.5,10],power:[1,2.5,5.4,9],detail:'A familiar little face grows into a towering guardian.'},
 {name:'Grand awakening',heights:[1,3.7,9,14],power:[1,3.1,7,12],detail:'New armor, sweeping crests, and elemental energy awaken with each stage.'},
 {name:'Total metamorphosis',heights:[1,4.5,13,20],power:[1,3.7,9,15],detail:'Each evolution transforms its body into something extraordinary.'},
] as const;
...
export function evolutionProfile(c:Creature){
 ...
 const ability=c.stage===4?(fourthEvolutions.find(form=>form.id===c.id)?.ability??`${c.type} transcendence`):(powers[c.type]??[`${c.type} spark`,`${c.type} surge`,`${c.type} dominion`])[stage];
