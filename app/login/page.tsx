...
export default function LoginPage() {
  ...
  useEffect(() => { fetch('/api/auth').then(r => r.json() as Promise<{inviteRequired?: boolean}>).then(d => setInviteRequired(d.inviteRequired === true)).catch(() => {}); }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const res = await fetch('/api/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: mode, email, password, inviteCode, displayName: mode === 'signup' ? displayName : undefined }) });
